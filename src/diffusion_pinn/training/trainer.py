...
def deterministic_train_pinn_log_d(pinn: 'DiffusionPINN',
    ...
    Enhanced to handle log(D) optimization and provide better monitoring
    """
    ...
    # Log(D) bounds for monitoring (these are handled inside the PINN model)
        ...
        for phase_idx, phase in enumerate(phase_configs):
            ...
            for epoch in range(phase['epochs']):
                ...
                with tf.GradientTape() as tape:
                    ...
                    # L2 regularization on network weights (not on log_D)
                ...
                # The PINN model handles log(D) bounds internally through clipping
                # No manual bounds enforcement needed here
                ...
                if epoch_counter % 100 == 0:
                    ...
                    if len(log_D_history) >= 50:
                        ...
                        # For log(D), we check absolute standard deviation
                        # since log(D) can be negative
                        ...
                        # Convergence criteria: log(D) standard deviation < 0.01
...
def compute_enhanced_log_d_regularization(pinn):
    ...
    # Most diffusion coefficients are between 1e-8 and 1e-3
    ...
    # 2. Soft bounds penalty (redundant with model's internal clipping, but helps gradients)
    log_D_min = -18.0
    log_D_max = -4.0
    ...
    stability_penalty = 0.00001 * tf.square(current_log_D + 9.0)  # Slight bias toward log(D) ≈ -9
...
def train_pinn(pinn: 'DiffusionPINN',
    ...
    if hasattr(pinn, 'log_D'):
        ...
        # Fall back to the existing deterministic training
        from .trainer import deterministic_train_pinn
...
def create_and_initialize_pinn(inputfile: str,
                             ...
                             seed: int = PINN_VARIABLES['random_seed'],
                             use_log_d: bool = True) -> Tuple['DiffusionPINN', Dict[str, tf.Tensor]]:
    """
    Create and initialize PINN with option for logarithmic D parameterization
    ...
    Args:
        ...
        use_log_d: Whether to use logarithmic D parameterization
    ...
    if use_log_d:
        # Use the new logarithmic PINN
        print("Creating PINN with logarithmic D parameterization...")
        pinn = DiffusionPINN(
        ...
        # Use the standard PINN (fallback)
        print("Creating PINN with standard D parameterization...")
        from ..models.pinn_standard import DiffusionPINN as StandardPINN
        pinn = StandardPINN(
            spatial_bounds=domain_info['spatial_bounds'],
            time_bounds=domain_info['time_bounds'],
            initial_D=initial_D,
